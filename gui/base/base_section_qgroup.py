...
class BaseSectionQGroup(QGroupBox):
    ...
    2. Direct instantiation – pass them as keyword arguments.
       ...
       inline (title, ui_dict, toml_key[, formula]) tuples.
    """

    title = ""
    params_ui_dict = {}
    toml_section_key = ""
    formula = None          # optional (values, config) -> latex line (see BaseSectionWidget)
    ...
    def __init__(self, parent, update_cache_fn, load_toml_fn, *,
        ...
        _formula = formula if formula is not None else type(self).formula
        ...
        self.changed = self.section_widget.changed
        layout = QVBoxLayout()
    ...
    def refresh_formula(self):
        self.section_widget.refresh_formula()
