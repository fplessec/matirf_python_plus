...
class ProfilesViewer(QWidget):
    ...
    ## replaces the image data and redraws the profiles:
    def set_image(self, image):
        self.image = image
        ## fast path: update existing AxesImage data without clearing the figure:
        if hasattr(self, '_imshow_yz') and self._imshow_yz is not None:
            image_cpu = image.clone().cpu().detach()
            vmin = min(image_cpu.mean(dim=2).min(), image_cpu.mean(dim=1).min()).item()
            vmax = max(image_cpu.mean(dim=2).max(), image_cpu.mean(dim=1).max()).item()
            self._imshow_yz.set_data(image_cpu.mean(dim=2).transpose(0, 1).numpy())
            self._imshow_yz.set_clim(vmin, vmax)
            self._imshow_zx.set_data(image_cpu.mean(dim=1).numpy())
            self._imshow_zx.set_clim(vmin, vmax)
    ...
    def update_plot(self):
        ...
        self._imshow_yz = None
        self._imshow_zx = None
        self.figure.clear()
        self.plot_profiles()
