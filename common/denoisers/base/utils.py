...
def _gaussian_1d(sigma, truncate=3.0):
    ...
    x = torch.arange(-r, r+1, device=settings.device, dtype=settings.dtype)
...
def _conv_reflect_5d(y5D, K):
    if K.dim() == 3:
        ...
        w = K.view(1,1,kZ,kY,kX).to(device=settings.device, dtype=settings.dtype)
        return F.conv3d(F.pad(y5D, pad, mode='reflect'), w)
    else:
        kY, kX = K.shape
        pad = (kX//2, kX//2, kY//2, kY//2, 0, 0)
        w = K.view(1,1,1,kY,kX).to(device=settings.device, dtype=settings.dtype)
        return F.conv3d(F.pad(y5D, pad, mode='reflect'), w)