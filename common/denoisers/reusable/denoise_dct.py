...
def _dct_matrix(N):
    n = torch.arange(N, device=settings.device, dtype=settings.dtype).view(1, -1)
    k = torch.arange(N, device=settings.device, dtype=settings.dtype).view(-1, 1)
...
class DCTDenoiser(Denoiser):
    ...
    def denoise(self, y, sigma, delta=1.0):
        ...
        Y = _dct_nd(y)
        N = y.numel()
        T = sigma * torch.sqrt(2 * torch.log(torch.tensor(N, device=settings.device, dtype=settings.dtype)))
