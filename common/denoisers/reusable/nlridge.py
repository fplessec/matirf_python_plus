...
class NLRidge(nn.Module):
    ...
    def block_matching(input_x, k, p, w, s):
        ...
        indices = torch.empty(N, ind_H_ref.size(0), ind_W_ref.size(0), k, dtype=ind_H_ref.dtype,
                              device=ind_H_ref.device)
        ...
        if (W - p + 1) % s != 1:
            indices[:, :Href, Wref:, :] = block_matching_aux(input_x_pad[:, :, :, -(2 * v + p):], k, p, v, s)
            if (H - p + 1) % s != 1:
                indices[:, Href:, Wref:, :] = block_matching_aux(input_x_pad[:, :, -(2 * v + p):, -(2 * v + p):], k, p,
                                                                 v, s)
        ...
        indices = (ind_row + rearrange(ind_H_ref, 'h -> 1 h 1 1')) * (W - p + 1) + (
                    ind_col + rearrange(ind_W_ref, 'w -> 1 1 w 1'))
        return indices
    ...
    def variance_groups(self, X, indices, p):
        ...
        if self.noise_type == 'gaussian-homoscedastic':
            ...
            raise ValueError(
                'noise_type must be either gaussian-homoscedastic, gaussian-heteroscedastic, poisson or poisson-gaussian.')
        return V
            ...
            for _ in range(1000):
                for i in range(k):
                    if self.constraints == 'conical':
                        ...
                        alpha = -((Q[..., i:i + 1, :] - Q[..., j:j + 1, :]) @ theta + C[..., i:i + 1, :] - C[...,
                                                                                                           j:j + 1,
                                                                                                           :]) / \
...
class NLRidgeDenoiser(Denoiser):

    name = "Non-Local Ridge"
    supports_3d = False
    ...
    def denoise(self, y, sigma, delta=1.0):
        ...
        noise_type = "gaussian-homoscedastic"
        if sigma <= 15:
            z = model(f4d, sigma=sigma, noise_type=noise_type, p1=7, p2=7, k1=18, k2=55,
                      w=37, s=4)
        elif sigma <= 35:
            z = model(f4d, sigma=sigma, noise_type=noise_type, p1=9, p2=9, k1=18, k2=90,
                      w=37, s=4)
        else:
            z = model(f4d, sigma=sigma, noise_type=noise_type, p1=11, p2=9, k1=20, k2=120,
                      w=37, s=4)
        return z.squeeze(0)  # back to (1, Y, X)
...
if __name__=="__main__":
    ...
    denoiser = NLRidgeDenoiser()
    g_denoised = denoiser(g_noisy, sigma_est)
