...
def denoise_NL_RIDGE(f, sigma):
    model = NLRidge()
    noise_type = "gaussian-homoscedastic"
    if sigma <= 15:
        z = model(f.unsqueeze(1), sigma=sigma, noise_type=noise_type, p1=7, p2=7, k1=18, k2=55,
                  w=37, s=4)
    elif sigma <= 35:
        z = model(f.unsqueeze(1), sigma=sigma, noise_type=noise_type, p1=9, p2=9, k1=18, k2=90,
                    w=37, s=4)
    else:
        z = model(f.unsqueeze(1), sigma=sigma, noise_type=noise_type, p1=11, p2=9, k1=20, k2=120,
                    w=37, s=4)
    z = z.squeeze()
    return z
...
if __name__=="__main__":
    ...
    g_denoised = denoise_NL_RIDGE(g_noisy, sigma_est)
