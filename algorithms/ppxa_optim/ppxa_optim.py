...
from algorithms.ppxa_optim.utils import prox_positivity, prox_regularization, compute_loss
...
class PpxaOptimization(Algorithm):
    ...
    def run(self, g, H, params: Dict[str, Any]):
        ...
        gamma_prox_estimated = 1 / torch.linalg.norm(H)**2
        ...
        #P = torch.inverse((identity + gamma * HtH))
        P = torch.inverse(identity + gamma * (1 - lambda_reg) * HtH) #NEW

        # f initial: f0 = ( HtH + λ_rr Id )^(-1) Htg   (ridge regression)
        lambda_rr = 10000.
        f = apply_matirf_operator(torch.inverse(HtH + lambda_rr * identity), Htg)
        #f = apply_matirf_operator(P, gamma*Htg)
        # PPXA inital terms
        ...
        #lambda_relax = float(lambda_relax)

        t0 = time.time()
        loss = compute_loss(f, g, H, reg, lambda_reg, rho, delta=delta)
        self._print(
            f"iter 0 \nloss={loss:.6e} | "
            f"lambda_relax={lambda_relax:.3g}"
        )
        prev_loss = loss
        ...
        for iter in range(max_iter):
            ...
            # calcul of the 3 ppxa terms (proximal operators) :

            #p[0] = apply_matirf_operator(P, u[0] + Htg)  # data fidelity prox
            p[0] = apply_matirf_operator(P, u[0] + gamma * (1 - lambda_reg) * Htg) #NEW

            #p[1] = prox_regularization(u[1], reg, lambda_reg, delta=delta, rho=rho)  # regularization prox
            p[1] = prox_regularization(u[1], reg, gamma * lambda_reg, delta=delta, rho=rho) #NEW

            p[2] = prox_positivity(u[2])  # positivity constraint prox
