...
Two proposal schemes, differing only in the order of the noise and data-consistency steps:

    D (default)   data consistency -> noise -> positivity -> denoising
    C             noise -> data consistency -> positivity -> denoising

Both correct the current iterate toward the data with a ridge inverse of the residual.
...
MCMC_UI_PARAMS = {
    ...
    "proposal_method": {
        ...
        "type": "option",
        "param_info": {"options_list": PROPOSAL_METHODS},
    },
    "lambda_rr": {
...
class Mcmc(Solver):
    ...
    def solve(self, objective, f0, params):
        ...
        self.report(f"MCMC | T={max_iter} | sigma={sigma} | beta={beta} | "
                    f"proposal={method} | denoiser={name}")
        ...
        for k in range(1, max_iter + 1):
            ...
            candidate = self._propose(objective, f, sigma, lambda_rr, method, denoiser_fn)
    ...
    def _propose(self, objective, f, sigma, lambda_rr, method, denoiser_fn):
        """
        Build a candidate from the current iterate.

        Both schemes combine the same three moves — a random perturbation, a pull toward
        data consistency, and a denoising — and differ only in whether the noise is added
        before or after the data correction.
        """
        operator = objective.operator
        if method == "C":
            z = f + sigma * torch.randn_like(f)
            z = z + operator.ridge_inverse(objective.g - operator.apply(z), lambda_rr)
        else:                                       # "D", the default
