"""
MCMC — an MMSE estimator: the mean of a Markov chain of plausible reconstructions.
...
CHANGES FROM V1, all measured before being made:
    > `proposal_method` ("C": noise before the data step, "D": after) is gone. Both gave the
      same reconstructions to three decimals, with and without denoisers. The remaining
      order (data step, noise, denoise) is v1's default "D", and the coherent one: the
      denoiser removes noise at exactly the level just injected.
    > beta and sigma are relative (see above), so values carry over between problems and
      noise levels. v1's absolute values in old configs are not read.
